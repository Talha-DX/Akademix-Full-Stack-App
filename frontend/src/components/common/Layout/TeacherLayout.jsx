...
import { teacherSidebar, flattenNav } from '../../../data/mockData'


/** Page frame for every teacher portal screen: sidebar + top bar + content. */
export default function TeacherLayout({ active, onNavigate, userName, userMeta, children }) {
  ...
  const title = flattenNav(teacherSidebar).find((n) => n.key === active)?.label ?? 'Overview'
    ...
    <div className="min-h-screen bg-surface">
      <div className="flex">
        ...
        >
          ...
          {!collapsed && <p className="hidden px-6 pt-4 text-xs font-mono uppercase tracking-[0.14em] text-white/50 lg:block">Teacher portal</p>}
          <p className="px-6 pt-4 text-xs font-mono uppercase tracking-[0.14em] text-white/50 lg:hidden">Teacher portal</p>

          <TeacherSidebar active={active} onNavigate={handleNavigate} collapsed={collapsed} onRequestExpand={handleRequestExpand} />
